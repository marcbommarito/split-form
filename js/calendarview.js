...
Calendar.handleMouseDownEvent = function (event)
{
  ...
  if (event.target.hasClassName('calendar-month-select') || event.target.hasClassName('calendar-year-select')) return;
...
Calendar.handleMouseUpEvent = function (event, key) {
  ...
  // User may click on nested calendar arrow (span element) which is inside the button.
  // We need to reassign 'el' to the button to access the '.calendar' property.
  if (el.classList.contains('calendar-arrow')) {
  ...
  if (title && nextYear && nextMonth && prevYear && prevMonth) {
    ...
    if (calendarNewMonth && calendarNewYear) {
      ...
      function createSelectHeader({
        ...
        th.setAttribute('style', 'background-color: transparent !important; color: inherit; font-size: 16px; font-weight: 500;');
        ...
        // Create select input with its own change handlers. We assign the class "unselectable" so Calendar.handleMouseUpEvent ignores this inputs events 
        ...
        select.addEventListener('change', function setDate() {
          ...
          if (calendar.triggerElement && calendar.triggerInputElement) {
            const dateValue = new Date(calendar.triggerInputElement.value);
            const ariaLabelDate = !isNaN(dateValue) ? 'Change date, ' + dateValue.toLocaleDateString('en-US', {
              weekday: 'long',
              year: 'numeric',
              month: 'long',
              day: 'numeric'
...
Calendar.prototype = {
  ...
  create: function(parent, id)
  {
    ...
    this.container.addEventListener('keydown', this.handleDayKeydown.bind(this));
  ...
  handleDayKeydown: function(e) {
    ...
    if (e.shiftKey && e.key === 'Tab') {
      const nextMonthButton = calendarNode.querySelector('button.nextMonth');
      const previousYearButton = calendarNode.querySelector('button.previousYear');
      /** @type {HTMLButtonElement | null} */
      const lastSelectedDay = calendarNode.querySelector('tbody tr button[tabIndex="0"]');

      if (document.activeElement === nextMonthButton && lastSelectedDay) {
        setTimeout(() => {
          lastSelectedDay.focus();
        }, 0);
      }
    ...
    const selectedDate = calendarNode.querySelector('td.selected button');
    if ((e.key === ' ' || e.key === 'Enter') && selectedDate) {
      selectedDate.focus();
      return;
    }
